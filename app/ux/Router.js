 ...
 * 
 * Enables routing engine for Ext JS 4 MVC architecture.
 * Responsible for parsing URI Token and fire a dispatch action
 * process. Uses Ext.History internally to detect URI Token changes,
 * providing browser history navigation capabilities.
 * 
 *      Ext.application({
 *          name: 'MyApp',
 *          ...
...
Ext.define('Ext.ux.Router', {
        ...
        init: function (app) {
            ...
            me.addEvents(
                 ...
                 * Fires before loading controller and calling its action.
                 * Handlers can return false to cancel the dispatch
                 * process.
        ...
        routeMatcher: function (app, route, rules) {
            ...
            if (!routeObj.controller && Ext.isDefined(Ext.global.console)) {
                Ext.global.console.error(
                    '[Ext.ux.Router] Config `controller` can`t be undefined', route, rules);
            }

            if (!routeObj.action && Ext.isDefined(Ext.global.console)) {
                Ext.global.console.error(
                    '[Ext.ux.Router] Config `action` can`t be undefined', route, rules);
            }
            //</debug>
        ...
        parse: function (token) {
            /* jshint maxdepth:5, maxcomplexity: 15, maxstatements: 36 */
            if (Ext.ux.Router.pause) {
                return false;
            }
            ...
            for (; i < len; i++) {
                ...
                if (route.regex) {
                    ...
                    matches = route.manageArgs ? token.match(route.matcher) :
                        tokenWithoutArgs.match(route.matcher);

                    // special index rule
                    if (tokenWithoutArgs === '' && route.route === '/' ||
                        tokenWithoutArgs === '/' && route.route === '') {
                        matches = [];
                    }
        ...
        dispatch: function (token, route, params) {
            ...
            if (!controller[route.action] && Ext.isDefined(Ext.global.console)) {
                Ext.global.console.error(
                    '[Ext.ux.Router] Controller action not found ', route.controller, route.action);
                return false;
            }
            //</debug>
