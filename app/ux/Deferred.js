...
Ext.define('Ext.ux.Deferred', {
    statics: {
         ...
         * @param {Ext.ux.Deferred/Ext.ux.Deferred[]/Function/Function[]} args One or more Ext.ux.Deferred
         * or Function.
         * If Function is given, it has to return an Ext.ux.Deferred or an error would be raised.
            ...
            for (var i = 0; i < promises.length; i++) {
                (function (i) {
                    ...
                    if (!(promise instanceof Ext.ux.Deferred)) {
                        Ext.Error.raise('Ext.ux.Deferred expected: given ' + typeof promise);
                    }
    ...
    reject: function () {
        ...
        if (result instanceof Ext.ux.Deferred) {
            ...
            if (typeof me.lastDfd !== 'undefined') {
                if (result) {
                    me.lastDfd.reject(result);
                } else {
                    me.lastDfd.reject.apply(me.lastDfd, arguments);
                }
            }
        }
    ...
    rejectImmediately: function () {
        var args = arguments, me = this;
        setTimeout(function () {
            me.reject.apply(me, args);
        }, 0);
    },
    ...
    resolve: function () {
        ...
        if (result instanceof Ext.ux.Deferred) {
            ...
            if (typeof me.lastDfd !== 'undefined') {
                if (result) {
                    me.lastDfd.resolve(result);
                } else {
                    me.lastDfd.resolve.apply(me.lastDfd, arguments);
                }
            }
        }
    ...
    /**
     * @method then
     * Attach on done and/or on fail functions.
     * The given functions will be called on 'resolve'/'reject' situation.
     ...
     * @param [ctx]
     */
    then: function (onDone, onFail, ctx) {
        var me = this,
            dfd = Ext.create('Ext.ux.Deferred');

        if (ctx) {
            me.done(onDone.bind(ctx)).fail(onFail.bind(ctx));
        } else {
            me.done(onDone).fail(onFail);
        }
